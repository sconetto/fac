...
le_float:
  ...
  lwc1 $f4, positivo           # Carrega para o coproc 1 o valor de positivo (no registrador $f6)
  lwc1 $f6, negativo           # Carrega para o coproc 1 o valor de negativo (no registrador $f8)
  lwc1 $f8, pi                 # Carrega para o coproc 1 o valor de pi  (no registrador $f10)
  lwc1 $f10, precisao           # Carrega para o coproc 1 o valor da precisão desejada (no registrador $f26)
  ...
  add.s $f12, $f0, $f2          # Adiciona o valor lido que está em $f0 em $f1, pois $f2 é zero

verifica_intervalo:
  c.lt.s $f12, $f2              # Verifica se o número lido que está em $f1 é menor que zero ($f2)
  bc1t intervalo_negativo      # Caso a verificação acima seja verdadeira pule para intervalo negativo
  c.lt.s $f2, $f12              # Verifica se zero ($f2) é menor que o número lido ($f1)
  bc1t intervalo_positivo      # Caso a verificação acima seja verdadeira pule para intervalo positivo

intervalo_positivo:
  c.le.s $f12, $f4              # Verifica se o número lido ($f1) é menor ou igual a 1 positivo ($f3)
...
intervalo_negativo:
  c.le.s $f6, $f12              # Verifica se 1 negativo ($f4) é menor ou igual ao número lido ($f1)
  mul.s $f10, $f10, $f6          # Caso o número seja negativo inverte a precisão para negativa, caso contrário no primeiro termo o An já será menor que a precisão
...
calc_arcsen:
  ...
  add.s $f14, $f2, $f4          # Inicializando o meu registrador do dividendo (com 1)
  add.s $f16, $f2, $f4          # Inicializando o meu registrador do divisor (com 1)
  add.s $f18, $f2, $f2         # Inicializando o meu registrador do termo An (com 0)
  add.s $f20, $f2, $f2         # Inicializando o meu registrador da soma total (com 0)
  add.s $f22, $f2, $f4         # Inicializando o meu registrador auxiliar de cálculo de fatorial (com 1)
  add.s $f24, $f2, $f4         # Inicializando o meu registrador auxiliar de cálculo de potências (com 1)
  # $f31 = x (valor lido)
  add.s $f30, $f12, $f2         # Cópia do valor lido x


loop_calc_arcsen:
  mul $t3, $t0, 2              # Calculo o valor de 2n
  jal fatorial                 # Calcula o fatorial de 2n, o resultado estará no registrador $f13
  mul.s	$f14, $f14, $f22         # Coloco no meu dividendo o valor de 2n!
  add.s $f22, $f2, $f4         # Reinicializo meu auxiliar de fatoriais
  add $t3, $zero, $t0          # Calculo o valor de n
  jal fatorial                 # Calcula o fatorial de n, o resultado estará no registrador $f13
  mul.s $f16, $f16, $f22         # Coloco no meu divisor n!
  add.s $f22, $f2, $f4         # Reinicializo meu auxiliar de fatoriais
  mul.s $f16, $f16, $f16          # Como o meu divisor só tem n!, calculo o valor de (n!)^2
  mul $t3, $t0, 2              # Volto a calcular o valor de 2n
  addi $t3, $t3, 1             # Calculo o valor de 2n + 1
  mtc1 $t3, $f26               # Movendo o valor de 2n + 1 para o coproc 1
  cvt.s.w $f26, $f26           # Convertendo para float o valor de $f15
  mul.s $f16, $f16, $f26         # Calculo no meu divisor a equação ((n!)^2)*(2n+1)
  ...
  mul.s $f16, $f16, $f24         # Calcula o resultado do divisor completo (((n!)^2)*(2n+1))*(4^n)
  add.s $f24, $f2, $f4         # Reinicializo meu auxiliar de potências
  mul $t3, $t0, 2              # Volto a calcular o valor de 2n
  ...
  add.s $f18, $f2, $f12         # Adiciono no An o meu dividendo
  div.s $f18, $f18, $f14        # Agora divido o dividendo pelo divisor
  mul.s $f18, $f18, $f24       # Agora multiplico pelo resultado da potência de x ^ (2n+1)
  # Temos então em $f10 ((2n!)\((4^n)*(n!^2)*(2n+1)))*(x^(2n+1)) que é o termo An da minha série
  add.s $f20, $f20, $f18       # Minha soma (registrador $f11) vai receber o que já tem acumulado mais o termo An
  ...
  add.s $f12, $f2, $f4          # Reinicializo meu dividendo
  add.s $f16, $f2, $f4          # Reincializo meu divisor
  add.s $f18, $f2, $f2         # Reinicializo meu termo An
  add.s $f22, $f2, $f4         # Reinicializo meu axuliar de fatorial
  add.s $f24, $f2, $f4         # Reinicializo meu auxiliar de potênias
...
fatorial:
  ...
  mtc1 $t3, $f26               # Movendo o valor de 2n para o auxiliar geral $f15
  cvt.s.w $f26, $f26           # Convertendo o valor de $f15 para float

loop_fatorial:
  mul.s $f22, $f26, $f26       # Multiplica o fatorial pelo termo
  add.s $f26, $f26, $f6        # Decrementa o termo para a próxima iteração
...
potencia:
  blez $t0, retorna_pc         # Verifico na chamada do fatorial se o meu termo é zero, caso sim eu retorno a execução por 0! = 1 e $f13 que é a auxiliar de fatoriais já é 1
  lwc1 $f28, const             # Carrego no meu auxiliar a constante 4

loop_potencia:
  mul.s $f24, $f24, $f28       # $f14 vai receber o seu valor atual vezes 4 a fim de calcular a potência
...
loop_potencia_x:
  mul.s $f24, $f24, $f12        # #$f2 = $f2 * x - cada iteração o $f2 será multiplicado por x.
...
compara_precisao:
  c.le.s $f10, $f2              # Se minha precisão for negativa eu seto a flag true, se não eu seto false
  bc1t compara_negativo        # Se a flag for true comparo negativo
  bc1f compara_positivo        # Se a flag for false comparo positivo

compara_negativo:
  c.le.s $f10, $f18             # Verifica se o termo an é menor que a precisão desejada
  j retorna_pc                 # Retorna a chamada da função

compara_positivo:
  c.le.s $f18, $f10             # Verifica se o termo an é menor que a precisão desejada
  j retorna_pc                 # Retorna a chamada da função
...
imprime_saida:
  ...
  syscall                      # Chamada de sistema
  add.s $f12, $f0, $f2         # Adiciona o valor lido que está em $f1 em $f12, pois $f2 é zero
  li $v0, 2                    # Carrega o imediato 2 (impressão de float) no registrador $v0
  syscall                      # Chamada de sistema
  li $v0, 4                    # Carrega o imediato 4 (impressão de string) no registrador $v0
  la $a0, msg_3                # Carrega em $a0 o meio da mensagem de saída
  syscall                      # Chamada de sistema
  add.s $f12, $f20, $f2        # Adiciona o valor da soma que está em $f11 em $f12, pois $f2 é zero
  li $v0, 2                    # Carrega o imediato 2 (impressão de float) no registrador $v0
  syscall                      # Chamada de sistema
  li $v0, 4                    # Carrega o imediato 4 (impressão de string) no registrador $v0
